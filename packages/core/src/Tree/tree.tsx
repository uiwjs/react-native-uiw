...
export type TreeProps = {
  ...
  defaultValue?: string[];
  value?: string[];
  onChange?: (value: string[], extend: { options: TreeOption[] }) => void;
...
export const Tree: FC<TreeProps> = (p) => {
  ...
  const [deep, optionsMap, optionsParentMap] = useMemo(() => {
    ...
    const optionsMap = new Map<string, TreeOption>();
    const optionsParentMap = new Map<string, TreeOption | undefined>();

    function traverse(current: TreeOption | undefined, children: TreeOption[]) {
  ...
  const onItemSelect = (node: TreeOption) => {
    // 找到父级节点
    const parentNodes: TreeOption[] = [];
    let current: TreeOption | undefined = node;
    while (current) {
  ...
  const renderItems = (columnOptions: TreeOption[] = [], index: number) => {
    return columnOptions.map((item) => {
      ...
      return (
        <TouchableOpacity
          ...
          key={item[valueName]}
          onPress={() => {
            console.log('isActive', isActive, 'item==>', item);
            if (!isActive) {
              onItemSelect(item);
            }
          }}
        ...
        >
          ...
          >
            ...
            <Text>
              <Icon name={isActive ? 'up' : 'down'} color="#ccc" size={16} />
            </Text>
          </View>
        </TouchableOpacity>
      );
  ...
  const renderColumns = () => {
    ...
    for (let i = 0; i < deep; i++) {
      ...
      const column = (
        <ScrollView
          key={i}
          style={{ width, flex: 1, backgroundColor: i === 0 ? theme.colors.gray100 : theme.colors.mask }}
        >
  ...
  return (
    <React.Fragment>
      ...
      <Modal visible={visible} onClosed={() => setVisible(false)} {...props.modalProps}>
        ...
        <View style={{ display: 'flex', width: '100%' }}>{renderColumns()}</View>
      </Modal>
    </React.Fragment>
  );
