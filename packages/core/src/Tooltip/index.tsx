...
export default class Tooltip extends React.Component<TooltipProps, TooltipState> {
  ...
  onOpen = () => {
    this.setState({ modalVisible: true }, () => {
      this.refFollow.current && this.refFollow.current.measure((_frameOffsetX, _frameOffsetY, _width, _height, pageOffsetX, pageOffsetY) => {
        this.refCloud.current && this.refCloud.current.measure((_X, _Y, _W, _H, _PX, _PY) => {
          this.setState({ modalViewStyle: { width: _width,height: _height, position: 'absolute', zIndex: 9999, left: pageOffsetX, top: pageOffsetY } })
  ...
  render() {
    const { props: { children, title, toggleAction = "onPress", backgroundColor = '#000', borderRadius = 10, ...other }, isDown, } = this
    return (
      <View>
        <Modal
        ...
        >
          ...
          <View style={[styles.followView, { ...this.state.modalViewStyle, }]}>
            {children}
          </View>
          <Animated.View
            style={[
          ...
          >
            <View
              style={[styles.containerView, { ...this.state.cloudStyle, }]}
              ref={this.refCloud}
            >
              <Cloud 
                title={title} 
