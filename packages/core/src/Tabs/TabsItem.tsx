...
function TabsItem(props: TabsItemProps) {
  ...
  const IconDom = useMemo(() => {
    const isIconDom = () => {
      if (typeof props.icon === 'string') {
        return <Icon name={props.icon as IconsName}
    ...
    if (props.icon) {
      return (
        <View style={[styles.iconBox, { ...style().iconBoxStyle }]}>
          {isIconDom()}
        </View>
      )
    } else return null
  ...
  const BorderDom = useMemo(() => {
    if (props.border) {
      return (<View style={styles.bottomView}><View style={[styles.bottom, { ...style().borderColor, }]} /></View>)
    } else return null
  }, [props.border])
