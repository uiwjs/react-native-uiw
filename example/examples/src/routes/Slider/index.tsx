        ...
        <Layout>
          ...
          <Body style={{paddingLeft: 16, paddingRight: 16}}>
            <Card title={`横竖垂直方向，value=${this.state.sliderValue.toFixed(2)}`} bodyStyle={{paddingLeft: 0, paddingRight: 0}}>
              <WingBlank>
                <Slider value={this.state.sliderValue as number} onChange={this.handleSliderChange} thumbSize={{width: 60, height: 30}} />
              </WingBlank>
              <WingBlank>
                <Slider vertical={true} style={{height: 200}} value={this.state.sliderValue as number} onChange={this.handleSliderChange} />
              </WingBlank>
            </Card>
            <Card title={`设置步长 step={0.2}，value=${this.state.stepValue.toFixed(1)}`} bodyStyle={{paddingLeft: 0, paddingRight: 0}}>
              <WingBlank>
                <Slider step={0.2} value={this.state.stepValue as number} onChange={this.handleStepSliderChange} />
              </WingBlank>
            </Card>
            <Card title={`不显示滑块 shownThumb={false}，value=${this.state.sliderValue.toFixed(2)}`} bodyStyle={{paddingLeft: 0, paddingRight: 0}}>
              <WingBlank>
                <Slider shownThumb={false} value={this.state.sliderValue as number} onChange={this.handleSliderChange} />
              </WingBlank>
            </Card>
            <Card title={'禁用 disabled={true}'} bodyStyle={{paddingLeft: 0, paddingRight: 0}}>
              <WingBlank>
                <Slider disabled={true} value={0.4} onChange={this.handleSliderChange} />
              </WingBlank>
            </Card>
          </Body>
          <Footer />
